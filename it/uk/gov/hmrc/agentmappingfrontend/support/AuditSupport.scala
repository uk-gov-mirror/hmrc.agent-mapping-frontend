...
trait AuditSupport extends BeforeAndAfterEach {
  ...
  def auditEventShouldNotHaveBeenSent[A](eventType: String): Unit =
    lastAuditEvent match {
      case Some(event) =>
        if (event.auditType == eventType)
          throw new TestFailedException(
            s"Unexpected audit event type $eventType : $event",
            0)
      case None => MatchResult.apply(matches = true, "", "")
    }

  def auditEventShouldHaveBeenSent[A](expectedAuditType: String)(
      matcher: Matcher[DataEvent]): Unit = lastAuditEvent match {
    case Some(event) if event.auditType == expectedAuditType =>
      val matcherResult = auditTagsNotEmpty("path",
                                            "X-Session-ID",
                                            "X-Request-ID",
                                            "clientIP",
                                            "clientPort")
    ...
    case None =>
      throw new TestFailedException(
        "Audit event has not been sent although expected.",
        0)
  }

  def auditSource(auditSource: String): Matcher[DataEvent] =
    new Matcher[DataEvent] {
      def apply(event: DataEvent): MatchResult =
        be(auditSource).apply(event.auditSource)
    }

  def auditType(auditType: String): Matcher[DataEvent] =
    new Matcher[DataEvent] {
      def apply(event: DataEvent): MatchResult =
        be(auditType).apply(event.auditType)
    }

  def auditDetailsNotEmpty(keys: String*): Matcher[DataEvent] =
    keys.map(k => auditDetail(k, not(be(empty)))).reduce(_ and _)

  def auditDetail(pair: (String, String)): Matcher[DataEvent] =
    auditDetail(pair._1, be(pair._2))

  def auditDetailKey(key: String)(
      implicit keyMapping: KeyMapping[Map[String, String]])
    : Matcher[DataEvent] = new Matcher[DataEvent] {
    def apply(event: DataEvent): MatchResult =
      contain.key(key).matcher(keyMapping).apply(event.detail)
  }

  def auditDetail(key: String, matcher: Matcher[String]): Matcher[DataEvent] =
    new Matcher[DataEvent] {
      def apply(event: DataEvent): MatchResult =
  ...
  def auditTagsNotEmpty(keys: String*): Matcher[DataEvent] =
    keys.map(k => auditTag(k, not(be(empty)))).reduce(_ and _)

  def auditTag(pair: (String, String)): Matcher[DataEvent] =
    auditTag(pair._1, be(pair._2))

  def auditTag(key: String, matcher: Matcher[String]): Matcher[DataEvent] =
    new Matcher[DataEvent] {
      def apply(event: DataEvent): MatchResult =
